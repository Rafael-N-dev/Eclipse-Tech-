...
const schema = z.object({
  ...
  slug: z.string().trim().min(3).max(200).regex(/^[a-z0-9-]+$/, "Use apenas a-z, 0-9 e -"),
...
export function PostForm({ initial }: { initial?: PostFormValues }) {
  ...
  const [v, setV] = useState<PostFormValues>(
    initial ?? {
      ...
      category: "curiosidades", reading_time: 3, published: true, featured: false,
    },
  );
  ...
  function autoSlug() {
    if (!v.slug && v.title) {
      set("slug", v.title.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <Input value={v.slug} onChange={(e) => set("slug", e.target.value)} placeholder="meu-post" />
        </div>
        <div>
      ...
      <div>
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Imagem de capa (URL)</label>
        <Input value={v.cover_image} onChange={(e) => set("cover_image", e.target.value)} placeholder="https://..." />
      </div>
      <div>
      ...
      <div>
        ...
        <Textarea rows={16} value={v.content} onChange={(e) => set("content", e.target.value)} className="font-mono text-sm" />
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground">Tempo de leitura (min)</label>
          <Input type="number" min={1} max={120} value={v.reading_time}
      ...
      <div className="flex gap-2">
        ...
        <Button type="button" variant="outline" onClick={() => navigate({ to: "/admin" })}>Cancelar</Button>
      </div>
    </form>
  );
}
